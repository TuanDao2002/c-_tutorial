...
public:
    ...
    //Overloading the -- operator  --object  --> return object (after -1) ; //unary prefix
    //e.g. (8 + 5i) --    --> (7 + 5i) 
    ComplNum operator -- () {
        this->real --;
    ...
    //Overloading the -- operator    object--  --> return object (before -1); //unary posfix
    ComplNum operator -- (int) {
        ComplNum result = *this; //get values of current object
        this->real --;

        return result; //result previous values
    }
...
int main() {
    ...
    std::cout << "num1 = "; num1.showInfo();
    std::cout << "num2 = "; num2.showInfo();
